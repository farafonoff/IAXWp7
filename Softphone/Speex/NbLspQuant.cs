namespace Ozeki.Media.Codec.Speex.Implementation
{
    ...
    internal class NbLspQuant : LspQuant
    {
        public sealed override void Quant(float[] lsp, float[] qlsp, int order, Bits bits)
        {
            ...
            data = LspQuant.Lsp_weight_quant(qlsp, 0, weight, 0, Codebook_Constants.cdbk_nb_low2, 0x40, 5);
            bits.Pack(data, 6);
            ...
            data = LspQuant.Lsp_weight_quant(qlsp, 5, weight, 5, Codebook_Constants.cdbk_nb_high2, 0x40, 5);
            bits.Pack(data, 6);
            for (num = 0; num < order; num++)
            {
                float? nullable2 = 0.00097656f;
